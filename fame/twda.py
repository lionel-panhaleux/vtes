...
HEADERS.update([
    '{}/{}'.format(h1, h2)
    for h1, h2 in itertools.permutations(HEADERS, 2)
])
...
class _TWDA(collections.OrderedDict):
    ...
    def _load_deck_html(self, lines, twda_id):
        ...
        if library_count < 60:
            logger.info(
                "[{:<6}] Deck #{} is missing library cards [{}]"
                .format(line_num, twda_id, current)
            )
        if library_count > 90:
            logger.info(
                "[{:<6}] Deck #{} has too many cards [{}]"
                .format(line_num, twda_id, current)
            )
        if current.cards_count(vtes.VTES.is_crypt) < 12:
            logger.info(
                "[{:<6}] Deck #{} is missing crypt cards [{}]"
                .format(line_num, twda_id, current)
            )
        return current
...
def _get_card(line):
    ...
    count = int(
        card_match.group('count') or
        card_match.group('count_or_cap') or
        1
    )
    return name, count
